...
const Body = ({searchQuery}) => {
      ...
      if (WeatherData.temp_c === "" && WeatherData.temp_f === "") {
        ...
        return (
          <div onClick={toggleTempreatureUnit}>
            {isCelsius ? (
              <>
                <span id="temp_main">{WeatherData.temp_c}°C</span><br />
                <span>(Click to change to Fahrenheit)</span><br />
                <span id="temp">Feels Like </span>
                <span id="temp_main">{WeatherData.feelslike_c}°C</span>
              </>
            ) : (
              <>
                <span id="temp_main">{WeatherData.temp_f}℉</span><br />
                <span>(Click to change to Celsius)</span><br />
                <span id="temp">Feels Like </span>
    ...
    const fetchdata = async (searchQuery) => {
        ...
        try{
        const data = await fetch("http://api.weatherapi.com/v1/current.json?key=" + API_Key +"&q="+ searchQuery +"&days=7&aqi=no&alerts=no");
  ...
  return (
    ...
    <span>{WeatherData.forecast_day1_date}</span>
  </div>
  );
};
